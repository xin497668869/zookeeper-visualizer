...
public class MyZkClient implements Watcher {
    ...
    private final static Logger LOG = LoggerFactory.getLogger(MyZkClient.class);
    ...
    public MyZkClient(String serverstring) {
        this(serverstring, Integer.MAX_VALUE);
    }

    public MyZkClient(String zkServers, int connectionTimeout) {
        this(new ZkConnection(zkServers), connectionTimeout);
    }

    public MyZkClient(String zkServers, int sessionTimeout, int connectionTimeout) {
     ...
     * @param operationRetryTimeout Most operations done through this {@link MyZkClient} are retried in cases like
     ...
     */
    public MyZkClient(final String zkServers, final int sessionTimeout, final int connectionTimeout, final ZkSerializer zkSerializer,
    ...
    public MyZkClient(IZkConnection connection) {
        this(connection, Integer.MAX_VALUE);
    }

    public MyZkClient(IZkConnection connection, int connectionTimeout) {
        this(connection, connectionTimeout, new SerializableSerializer());
    }

    public MyZkClient(IZkConnection zkConnection, int connectionTimeout, ZkSerializer zkSerializer) {
     ...
     * "retry forever until a connection has been reestablished".
     */
    public MyZkClient(final IZkConnection zkConnection, final int connectionTimeout, final ZkSerializer zkSerializer,
                      final long operationRetryTimeout) {
     ...
     *
     * @param createParents if true all parent dirs are created as well and no {@link ZkNodeExistsException} is thrown in case the
     * path already exists
     * @throws ZkInterruptedException if operation was interrupted, or a required reconnection got interrupted
     * @throws IllegalArgumentException if called from anything except the ZooKeeper event thread
     * @throws ZkException if any ZooKeeper exception occurred
     ...
     * @throws RuntimeException if any other exception occurs
     */
    public void createPersistent(String path, boolean createParents, List<ACL> acl) throws ZkInterruptedException, IllegalArgumentException,
     ...
     *
     * @param acl List of ACL permissions to assign to the path.
     * @throws ZkException if any ZooKeeper exception occurred
     * @throws RuntimeException if any other exception occurs
     */
     ...
     * @throws RuntimeException if any other exception occurs
     */
    public String createPersistentSequential(String path, Object data) throws ZkInterruptedException, IllegalArgumentException, ZkException,
                                                                              RuntimeException {
     ...
     * @throws RuntimeException if any other exception occurs
     */
    public String createPersistentSequential(String path, Object data, List<ACL> acl) throws ZkInterruptedException,
     ...
     * @throws RuntimeException if any other exception occurs
     */
    public void createEphemeral(final String path, final List<ACL> acl) throws ZkInterruptedException, IllegalArgumentException,
                                                                               ZkException, RuntimeException {
     ...
     * @throws RuntimeException if any other exception occurs
     */
    public void createEphemeral(final String path, final Object data, final List<ACL> acl) throws ZkInterruptedException,
                                                                                                  IllegalArgumentException, ZkException,
                                                                                                  RuntimeException {
     ...
     * @throws RuntimeException if any other exception occurs
     */
    public String createEphemeralSequential(final String path, final Object data, final List<ACL> acl) throws ZkInterruptedException,
    ...
    public void process(WatchedEvent event) {
        ...
        try {
            ...
            if (stateChanged) {
                getEventLock().getStateChangedCondition()
                              .signalAll();
                ...
                if (event.getState() == KeeperState.Expired) {
                    getEventLock().getZNodeEventCondition()
                                  .signalAll();
                    getEventLock().getDataChangedCondition()
                                  .signalAll();
    ...
    public void unsubscribeDataChanges(String path) {
        Set<IZkDataListener> iZkDataListeners = _dataListener.get(path);
        if (iZkDataListeners != null) {
            iZkDataListeners
                    .clear();
        }
    }
    ...
    public void showFolders(OutputStream output) {
        try {
            output.write(org.I0Itec.MyZkClient.MyZkPathUtil.toString(this)
                                                           .getBytes());
        ...
        while (true) {
            ...
            if (_operationRetryTimeoutInMillis > -1 && (System.currentTimeMillis() - operationStartTime) >= _operationRetryTimeoutInMillis) {
                throw new ZkTimeoutException(
                        "Operation cannot be retried because of retry timeout (" + _operationRetryTimeoutInMillis + " milli seconds)");
            }
        }
    }
    ...
    public void connect(final long maxMsToWaitUntilConnected, Watcher watcher) throws ZkInterruptedException, ZkTimeoutException,
        ...
        try {
            ...
            if (!waitSuccessful) {
                throw new ZkTimeoutException(
                        "Unable to connect to zookeeper server '" + _connection.getServers() + "' with timeout of " + maxMsToWaitUntilConnected + " ms");
            }
            started = true;
        } finally {
            getEventLock().unlock();
