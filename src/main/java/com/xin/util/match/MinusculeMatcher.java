...
public class MinusculeMatcher {
     ...
     */
    public MinusculeMatcher(String pattern, MatchingCaseSensitivity options, String hardSeparators) {
        myOptions = options;
        myPattern = ("*" + StringUtil.trimStart(StringUtil.trimEnd(pattern, "*"), "*") + "*").toCharArray();
    ...
    private static int nextWord(String name, int start) {
    ...
    public static boolean isStartMatch(Iterable<TextRange> fragments) {
    ...
    public int matchingDegree(String name, boolean valueStartCaseMatch, FList<TextRange> fragments) {
    ...
    private int evaluateCaseMatching(boolean valuedStartMatch,
        ...
        if (nameChar == myPattern[patternIndex]) {
            if (isUpperCase[patternIndex])
                return 50; // strongly prefer user's uppercase matching uppercase: they made an effort to press Shift
            if (nameIndex == 0 && valuedStartMatch)
                return 150; // the very first letter case distinguishes classes in Java etc
    ...
    private FList<TextRange> matchWildcards(String name,
                                            int patternIndex,
                                            int nameIndex,
                                            boolean isAsciiName) {
        if (nameIndex < 0) {
    ...
    private int findNextPatternCharOccurrence(String name,
                                              int startAt,
                                              int patternIndex,
                                              boolean isAsciiName,
    ...
    private boolean seemsLikeFragmentStart(String name, int patternIndex, int nextOccurrence) {
        ...
        return !isUpperCase[patternIndex] ||
                ...
                !myHasHumps && myOptions != MatchingCaseSensitivity.ALL;
    }
    ...
    private FList<TextRange> matchFragment(String name,
                                           int patternIndex,
                                           int nameIndex,
                                           boolean isAsciiName) {
    ...
    private int maxMatchingFragment(String name, int patternIndex, int nameIndex) {
        ...
        int i = 1;
        boolean ignoreCase = myOptions != MatchingCaseSensitivity.ALL;
    ...
    private boolean shouldProhibitCaseMismatch(String name, int patternIndex, int nameIndex) {
        ...
        if (myHasHumps && patternIndex >= 2 && isUpperCase[patternIndex - 1] && isUpperCase[patternIndex - 2]) {
            ...
            return nameIndex + 1 == name.length() ||
    ...
    private boolean isFirstCharMatching(String name, int nameIndex, int patternIndex) {
        ...
        return myOptions != MatchingCaseSensitivity.FIRST_LETTER ||
